...
import { TourManager } from './astarTesting'
...
export class Portal extends SceneItem{
  ...
  showAtEndOfSegment(tourManager:TourManager) {
    log(this.name,"showAtEndOfSegment",this.entity.getComponent(Transform).position)
    this.placeAtEndOfSegment(tourManager.getCurrentDayEndSegmentPositionAbs())
    this.show()
  }
  showAtStartOfSegment(tourManager:TourManager) {
    log(this.name,"showAtStartOfSegment",this.entity.getComponent(Transform).position)
    this.placeAtEndOfSegment(tourManager.getNextDayStartSegmentPositionAbs())
    this.show()
  }
}