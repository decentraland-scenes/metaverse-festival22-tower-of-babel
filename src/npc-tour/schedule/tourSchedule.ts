...
import { TourDataType, ShowMatchRangeResult, ShowResultType, TourDayType } from './types'
...
export class TourSchedule{
  private campaignData?:TourDataType
  campaigns?:TourDayType[]
...
/*
  ...
  findShowToPlayByDateInPlace(showMatch:ShowMatchRangeResult,date:Date,startIndex?:number):ShowMatchRangeResult{
    ...
    let showPlaying: TourDayType|undefined = undefined
    let counter = 0
    ...
    let nearestShowToNow:TourDayType|undefined
 ...
 * @returns 
 */
function inPlaceOrAssignShow(currentShow: ShowResultType|undefined, show: TourDayType, offset: number, index: number) {
  if(!currentShow){ 
