...
class PlayerWalkingSystem implements ISystem{
  ...
  update(dt: number): void {
    ...
    if(this.lastKnownPos === undefined ){
      //0,0,0 can be fired on scene initial load, so ignoring it
      if(playerCurrentPosition.x != 0 && playerCurrentPosition.y != 0 && playerCurrentPosition.z != 0){//} //ignore 0){
        this.lastKnownPos = playerCurrentPosition.clone()
      }else{
        //log("PlayerWalkingSystem.ignored using this value",playerCurrentPosition,this.lastKnownPos)   
      }
    }else{
      ...
      if(!playerCurrentPosition.equalsWithEpsilon(this.lastKnownPos,1)){
        //log("PlayerWalkingSystem.moved",playerCurrentPosition,this.lastKnownPos)   
